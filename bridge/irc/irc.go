...
func (b *Birc) handlePrivMsg(event *irc.Event) {
	...
	if strings.HasPrefix(msg, "!ignore ") {
		toIgnore := strings.Split(msg, " ")[1]
		b.Ignored = append(b.Ignored, toIgnore)
		return
	} else if strings.HasPrefix(msg, "!unignore ") {
		toUnignore := strings.Split(msg, " ")[1]
		for i, u := range b.Ignored {
			if u == toUnignore {
				b.Ignored = append(b.Ignored[:i], b.Ignored[i+1:]...)
			}
		}
		return
	}
